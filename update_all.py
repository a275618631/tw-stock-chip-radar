...
def get_common_latest_date(*frames: pd.DataFrame) -> date:
    """Return the latest date shared by all non-empty history tables.

    Public endpoints can publish the current day's data at slightly different
    times.  Validation should use the latest common trading date rather than
    failing an otherwise usable daily update when today's endpoint is not yet
    available.
    """
    latest_dates = []
    for frame in frames:
    ...
    if not latest_dates:
        raise RuntimeError("cannot determine a common latest trading date")
...
def main():
    ...
    if validation_target_date != target_date:
        print(
            f"[WARN] target date {target_date} is not available from every market; "
            f"validating common latest trading date {validation_target_date}."
        )
